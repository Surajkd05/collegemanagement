...
import Constants from "../../shared/apiIndex"
...
export default class PieChart extends React.Component {
  ...
  onSelectChange = (e) => {
    this.setState(
      { isLoading: true, selected: e.currentTarget.value },
      this.fetchData
    );
  };
          ...
          <select onChange={this.onSelectChange}>
            {options.map((option) => {
              return (
                <option
                  key={option.roomId}
                  defaultValue={option.roomId === selected}
                  value={option.roomId}
                >
                  {option.roomName}
                </option>
        ...
        <div className="commonbox__body">
          {isLoading ? (
            ...
            <HighchartsReact
              ...
              options={chartOptions(this.getSelectedOptionData())}
            />
          )}
        </div>
      </div>
