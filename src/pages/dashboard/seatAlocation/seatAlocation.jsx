...
class SeatAlocationView extends React.Component{
    ...
    onDoneClick = () => {
        ...
        const url = actionHasalocated ? Constants.API_URLS.GET_ALL_LOC_SEAT : Constants.API_URLS.GET_ALL_DELOC_SEAT;
        ...
        axios.post( url, {
            ...
            this.setState({ seatAlocated: true, isLoading: false }, () => {
                ...
                const _msg = actionHasalocated ? `Seat Allocated to the Employee Name: ${empName} Is ${this.getSeatCode()} Location as Gurgaon` : `Seat De-allocated to the Employee Name: ${empName} Location as Gurgaon`;
        ...
        if( ! isLoading ) {
            ...
            if( seatAlocated  ) {
                ...
                <div className='roombx roombx__modify'>
                    <p>Seat { ! actionHasalocated ? 'De-' : '' }allocated to {empName} {empId}. <br />
                    Employee will receive update on his mail shortly!.</p>
                </div>);
                    ...
                    <React.Fragment>
                        <div className='roombx'>
                            <div className='roombx__doors'>
                                { new Array( _data.doors ).fill( undefined ).map( ( _d, index ) =>  <div key={ index } className='roombx__door' /> ) }
                            </div>
                            <div className='container-fluid'>
                                <div className='row align-items-center justify-content-center'>
                                    <div className='col col-1 d-flex justify-content-center'><img src={WaveIcon} alt='waive' /></div>
                                    <div className='col col-1 d-flex justify-content-center'><img src={SystemIcom} alt='SystemIcom' /></div>
                                    <MakeRoom 
                                        { ...{
                        ...
                        <div className='row'>
                            <div className='col col-12 text-right'>
                                <button type="button" className="btn btn-sm btn-outline-secondary roombx__btn" disabled={ isEmpty( selectedSeatId ) } onClick={ this.onDoneClick }>Done</button>
                            </div>
                        </div>
                    </React.Fragment>
                );
            ...
            <div className='container-fluid seat'>
                <div className='row'>
                    <div className='col col-12'>
                        <h2 className="seat__title">Seat Allocation - { this.getProjectName() }</h2>
                    </div>
                </div>
                { view }
            </div>
